...
const buildLinks = (count, seed) => {
  ...
  for (let i = 0; i < count; i++) {
    const domain =
      domains[Math.floor(pseudoRand(seed + i * 31) * domains.length)];
    const path = paths[Math.floor(pseudoRand(seed + i * 57) * paths.length)];
    links.push(
      `http://${domain}/${path}/${Math.floor(pseudoRand(seed + i * 97) * 9000 + 1000)}`,
    );
  }
  return links.join(" ");
};
...
const variedEmailText = (item, idx) => {
  ...
  const signature = ["Alex", "Sam", "Taylor", "Jordan", "Casey"][
    Math.floor(pseudoRand(seed + 7) * 5)
  ];
  ...
  if (item.target === 0 && words < 60) {
    const subject =
      hamShortSubjects[
        Math.floor(pseudoRand(seed + 19) * hamShortSubjects.length)
      ];
  ...
  if (item.target === 0) {
    const subject =
      hamLongSubjects[
        Math.floor(pseudoRand(seed + 29) * hamLongSubjects.length)
      ];
    const body = [
      "Hi team,",
  ...
  // SPAM
  const subject =
    spamSubjects[Math.floor(pseudoRand(seed + 37) * spamSubjects.length)];
  const hooks = [
  ...
  const body = [
    ...
    linkBlock
      ? `Links: ${linkBlock}`
      : "Links: http://promo-deals.io/offer/1234",
    closing,
  ].join("\n");
  return `Subject: ${subject}\n${body}`;
};
...
export const config = {
  ...
  description:
    'Spam detection with a Hidden Layer (10 neurons). The Deep Neural Network can detect the nonlinear distribution ("Spam Island").',
